...
public final class MOSA {        
    ...
    public MOSA() throws IOException, ClassNotFoundException, Throwable {
        ...
        if (config.getSystemExec().equals(Constants.SYS_EXEC_GPS_ANALYSER)){
            monitoring();
        } else if (config.getSystemExec().equals(Constants.SYS_EXEC_POS_ANALYSER)){
            posAnalyser();
        } else if (config.getSystemExec().equals(Constants.SYS_EXEC_ROUTE_READY)){
            monitoring();
            sendRouteReadyToDronekit();
        } else if (config.getSystemExec().equals(Constants.SYS_EXEC_PLANNER)){
            monitoring();
            start();
            loop();
        }
...
//        mission.printMission();
        communication.setMission(mission);
        if (config.getRouteDynamic().equals("TRUE")){
            try {
                Thread.sleep(10000);
            ...
            readFileReadyRoute(config.getDirRouteReady()+ "route-4wp-dyn.txt", mission2);
//            mission2.printMission();
    ...
    private void posAnalyser() throws IOException{
        System.out.println("posAnalyser");        
        Executors.newSingleThreadExecutor().execute(new Runnable() {
            @Override
            public void run() {
                while(true){
